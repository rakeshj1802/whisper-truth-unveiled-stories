...
const CategoriesNav = () => {
  const { categorySlug } = useParams<{ categorySlug: string }>(); // Get current category slug
                  ...
                  <Link to={category.href}>
                    <NavigationMenuLink
                      className={cn(
                        ...
                        isActive && "bg-purple-600/30 text-purple-200 ring-1 ring-purple-500" // Active state styles
                      )}
                    >
                      {category.name}
                    </NavigationMenuLink>
